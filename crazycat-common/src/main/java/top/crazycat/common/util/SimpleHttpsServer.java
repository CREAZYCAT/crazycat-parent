...
public class SimpleHttpsServer {
    ...
    static ProcessResult runJavaProgram(String code) throws IOException, InterruptedException {
        ...
        String[] command = new String[] { "javac","-encoding","UTF-8","Main.java"};
        ...
        if (p.waitFor(5, TimeUnit.SECONDS)) {
            ...
            try (InputStream input = p.getInputStream()) {
                result = readAsString(input, Charset.forName("GBK"));
            }
            if(p.exitValue() != 0){
            ...
            System.err.println(String.format("Error: process %s timeout. destroy forcibly.", p.exitValue()));
            p.destroyForcibly();
            return new ProcessResult(p.exitValue(), "Timeout.");
        }
    }

    private static ProcessResult process2(File pwd) throws IOException, InterruptedException {
        String[] command = new String[] { "java","-Dfile.encoding=UTF-8", "Main"};
        System.out.println(String.format("cd %s\n%s", pwd.toString(), String.join(" ", command)));
        ProcessBuilder pb = new ProcessBuilder().command(command).directory(pwd);
        pb.redirectErrorStream(true);
        Process p = pb.start();
        if (p.waitFor(5, TimeUnit.SECONDS)) {
            String result = null;
            try (InputStream input = p.getInputStream()) {
                result = readAsString(input, Charset.forName("UTF-8"));
            }
            return new ProcessResult(p.exitValue(), result);
        } else {
            System.err.println(String.format("Error: process %s timeout. destroy forcibly.", p.exitValue()));
    ...
    static class CodeHandler implements HttpHandler {
        ...
        public void handle(HttpExchange exchange) throws IOException {
            ...
            if ("GET".equals(method)) {
                ...
                String body = readAsString(exchange.getRequestBody(), Charset.forName("UTF-8"));
                if (!body.startsWith("code=")) {
                    sendResult(exchange, 1, "No code found.");
                } else {
                    String code = URLDecoder.decode(body.substring(5), Charset.forName("UTF-8").name());
