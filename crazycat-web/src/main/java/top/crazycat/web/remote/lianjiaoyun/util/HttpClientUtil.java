package top.crazycat.web.remote.lianjiaoyun.util;
 ...
 * <p>
 * 系统版本: v1.0<br>
 * 开发人员: luopeng12856 开发时间: 2016—05-26 上午10:12 <br>
 */
...
//	public static PoolingHttpClientConnectionManager cm = new PoolingHttpClientConnectionManager();
	...
	public static String sendPost(String url, Map<String, String> params, String charSet, String charsetReturn,
		...
		try {
			...
			if (proxy != null) {
				...
				RequestConfig requestConfig = builder.setSocketTimeout(socketTimeout).setConnectTimeout(connectTimeout)
						.setConnectionRequestTimeout(requestTimeout).setExpectContinueEnabled(false)
			...
			try {
				...
				LogUtils.log("HttpClient   请求 http状态码 status = [" + status + "]  获取HttpEntity ", e, log);
			} finally {
				if (entity != null) {
	...
	public static String sendGet(String url, Map<String, String> params, String charSet, HttpHost proxy,
		...
		try {
			...
			RequestConfig defaultConfig = builder.setSocketTimeout(socketTimeout).setConnectTimeout(connectTimeout)
					.setConnectionRequestTimeout(requestTimeout).setExpectContinueEnabled(false)
					.setRedirectsEnabled(true).build();
	...
	public static String Base64(String appkey, String appsecret, String basic) throws UnsupportedEncodingException {
		...
		byte[] encodeBase64 = Base64.encodeBase64(str.getBytes(HttpClientUtil.CHARSET));
